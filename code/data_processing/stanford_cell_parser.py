...
def find_mapper_entry(meta_df, battery_id, temp_k):
    ...
    meta_df["_Battery_ID_norm"] = meta_df["Battery_ID"].astype(str).str.strip().str.lower()
    ...
    if cell_df.empty:
        ...
        cell_df = meta_df[meta_df["_Battery_ID_norm"].str.contains(battery_id.lower(), na=False)]
...
def filter_voltage_range(df, vmin, vmax, tolerance=0.01):
    ...
    voltage_mask = (df['Voltage(V)'] >= vmin - tolerance) & (df['Voltage(V)'] <= vmax + tolerance)
    ...
    if voltage_segments:
        ...
        return df.iloc[largest_segment[0]:largest_segment[1]+1].reset_index(drop=True)
    else:
        return df
    ...
    for i, cycle in enumerate(unique_cycles):
        ...
        if has_charge_data:
            ...
            if len(charge_cycle_df) > 0:
                charge_cycle_df.loc[:, "Charge_Time(s)"] = charge_cycle_df["Test_Time(s)"] - charge_cycle_df["Test_Time(s)"].iloc[0]
        ...
        if has_discharge_data:
            ...
            if len(discharge_cycle_df) > 0:
                discharge_cycle_df.loc[:, "Discharge_Time(s)"] = discharge_cycle_df["Test_Time(s)"] - discharge_cycle_df["Test_Time(s)"].iloc[0]
                
                # Generate discharge plot
                plt.figure(figsize=(10, 6))
                plt.plot(discharge_cycle_df['Discharge_Time(s)'], discharge_cycle_df['Voltage(V)'], 'r-')
...
def main():
    ...
    for group_key, files in file_groups.items():
        ...
        if cell_df.empty:
            ...
            cell_vmax = cell_df["Max_Voltage"].values[0] if not pd.isna(cell_df["Max_Voltage"].values[0]) else 3.6
        ...
        for i, file_info in enumerate(files):
            try:
                df = parse_file(file_info['path'], cell_initial_capacity, file_info['c_rate'], cell_vmin, cell_vmax)
