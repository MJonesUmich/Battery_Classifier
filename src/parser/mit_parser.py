...
def generate_figures(
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        df_discharge["Test_Time(s)"], df_discharge["Voltage(V)"], "r-"
...
def parse_meta_data(meta_df=None, battery_id_tag=None):
    ...
    if meta_df is None or battery_id_tag is None:
        ...
        cell_df = meta_df[
            meta_df["Battery_ID"].str.lower() == str.lower(battery_id_tag)
        ]
...
def scrub_data(
    ...
    if len(agg_df) > 0:
        agg_df.to_csv(
            f"{output_data_folder}\{battery_id_tag}_aggregated_dataset.csv", index=False
        )
...
def mit_parser(input_files, input_folder, meta_df, output_folder_paths):
    ...
    for cell_file in cell_datafiles:
        ...
        cell_C_rate_charge, cell_C_rate_discharge, battery_id_tag, cell_temperature = (
            parse_meta_data(
                meta_df=meta_df,
