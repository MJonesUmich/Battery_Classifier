...
def get_folders(parent_path, store_path):
    ...
    chemistry_folders = [
        ...
        if os.path.isdir(os.path.join(parent_path, name)) and name != store_path
    ]

    chemistry_folders = [
        os.path.join(parent_path, folder) for folder in chemistry_folders
    ]
...
def sample_data(chemistry_folders, lowest_count, store_path, seed=42):
    ...
    for folder in chemistry_folders:
        ...
        if folder != store_path:
            all_images = [
                image for image in os.listdir(folder) if image.endswith(".png")
            ]
            ...
            test_size = (
                lowest_count - train_size - val_size
            )  # ensures total = lowest_count
